...
//      Key key = Keys.hmacShaKeyFor();
    ...
    private static boolean verifyToken(String accessToken, final String secretKey) {
        boolean validation = Jwts.parser().setSigningKey(secretKey).parseClaimsJws(accessToken).getBody().getSubject().equals("{\"name\": \"jeongeun.choi\"}");
        if (!validation) {
